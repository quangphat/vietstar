...
GO
  ALTER PROCEDURE dbo.sp_HO_SO_CapNhat
@ID int,
...
if(@UpdatedDate is null) begin set @UpdatedDate = GETDATE() end;
	UPDATE HO_SO SET Ten_Khach_Hang=Upper(@TenKhachHang),
		...
		UpdatedTime = @UpdatedDate,Ma_Nguoi_Cap_Nhat = @UpdatedUserId,
		Ma_Ket_Qua=@KetQuaHS,
		BirthDay = @birthDay,
		CMNDDay= @cmndDay,
...
BEGIN
	...
	Update HO_SO Set HO_SO.Ma_Nguoi_Cap_Nhat = @NhanVienID, HO_SO.UpdatedTime = @NgayThaoTac, HO_SO.Da_Xoa = 1 
	Where HO_SO.ID = @ID
END
...
IF @@ERROR <> 0 SET NOEXEC ON
GO
PRINT N'Altering [dbo].[sp_HO_SO_CapNhatHoSo]'
GO
...
BEGIN
	UPDATE HO_SO SET Ten_Khach_Hang=Upper(@TenKhachHang),
		CMND=@CMND,Dia_Chi=@DiaChi, Courier_Code=@CourierCode,Ma_Khu_Vuc=@MaKhuVuc,SDT=@SDT,SDT2=@SDT2,Gioi_Tinh=@GioiTinh,
		Ngay_Nhan_Don=@NgayNhanDon, Ma_Nguoi_Cap_Nhat=@MaNguoiCapNhat,AssigneeId=@HoSoCuaAi,UpdatedTime=@NgayCapNhat,
		Ma_Trang_Thai=@MaTrangThai,San_Pham_Vay=@SanPhamVay,Co_Bao_Hiem=@CoBaoHiem,
		So_Tien_Vay=@SoTienVay,Han_Vay=@HanVay,Ten_Cua_Hang=@TenCuaHang,
		Ma_Ket_Qua=@KetQuaHS,
		BirthDay = @birthDay,
...
IF @@ERROR <> 0 SET NOEXEC ON
GO
PRINT N'Altering [dbo].[sp_HO_SO_CapNhatTrangThaiHS]'
GO
-- =============================================
-- Author:		<Author,,Name>
-- Create date: <Create Date,,>
...
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
	-- interfering with SELECT statements.
	Update HO_SO Set Ma_Nguoi_Cap_Nhat = @MaNguoiThaoTac, UpdatedTime = @NgayThaoTac, Ma_Trang_Thai = @MaTrangThai, Ma_Ket_Qua = @MaKetQua, Ghi_Chu = @GhiChu
	Where HO_SO.ID = @ID
END
...
BEGIN
 ...
 ,Ngay_Nhan_Don, Ma_Trang_Thai, Ma_Ket_Qua,San_Pham_Vay,Ten_Cua_Hang,Co_Bao_Hiem,So_Tien_Vay,Han_Vay,Da_Xoa, UpdatedTime,
